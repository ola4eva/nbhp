...
class StockScrap(models.Model):
    ...
    state = fields.Selection(selection_add=[('new', "Draft"), ('submit', "HOD To Approve"), ('hod', "Finance To Approve"), ('finance', "Finance Approved"), ('reject','Rejected'), ('done',)], default='new')

    def _default_employee(self):
        return self.env['hr.employee'].sudo().search([('user_id','=', self.env.uid)])
    
    requester_id = fields.Many2one(comodel_name='hr.employee', required=True, string='Requester', tracking=True, default=_default_employee, states={'draft': [('readonly', False)]})
    ...
    requester_hod_id = fields.Many2one(comodel_name='hr.employee', related='requester_department_id.manager_id', string='Requester HOD')
    ...
    def button_submit(self):
        ...
        self.name = self.env['ir.sequence'].next_by_code('stock.scrap') or _('New')
        message = "Scrap Order {} requires your (HOD) Approval".format(self.name)
        partners_to_notify = self.requester_hod_id.user_id.partner_id
        self.notify_of_scrap_order(message=message, partner_ids=partners_to_notify.ids)
        self.state = 'submit'
        return True

    def button_hod_approve(self):
        message = "HOD Approved, Scrap Order {} requires your (Finance) Approval".format(self.name)
