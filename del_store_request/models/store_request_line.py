...
class LineApprove(models.Model):
    ...
    state_main = fields.Selection(
        selection=REQUEST_STAGE, tracking=True, related='request_id.state', default='draft')
    ...
    to_procure = fields.Boolean(
        string='To Procure', compute='_compute_to_procure')
    ...
    def _compute_to_procure(self):
        for con in self:
            to_procure = (
                con.state == 'partially_available' or con.state == 'not_available')
    ...
    def _compute_qty(self):
        for con in self:
            ...
            stock_quants = con.env['stock.quant'].search(
                [('location_id', '=', location_id), ('product_id', '=', product_id)])
            con.qty = sum(
                [stock_quant.available_quantity for stock_quant in stock_quants])
